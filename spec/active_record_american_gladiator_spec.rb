...
describe "ActiveRecord American Gladiator" do
  context "Joust" do
    it "loads all items" do
      ...
      # Changeable Start
      items = Item.unscoped
      #items = Item.where("status = ?", "active OR inactive")
  ...
  context "Hang Tough" do
    it "returns orders for 3 users in 2 queries (aka: Remove the N+1 query)" do
      ...
      # users = User.includes(orders: { items: :amount }).first(3)
      users = User.includes(:orders).first(3)
      
      # The idea is to use select here to create a 'virtual attribute' 
      # That way, when you call user.orders in the expectation,
      # We don't have to go back to the DB to retrieve the information
      #
      # Changeable End
  ...
  context "Breakthrough and Conquer" do
    it "returns all orders with Footballs and Wrestling Rings" do
      ...
      # Here, the 'where' clause needs to specify the items table to look up the names
      # orders = Order.joins(:items).where({"items.name" => ["Wrestling Ring", "Football"]})

      orders = Order.joins(:items).
        where( { items: { name: ["Wrestling Ring", "Football"] } } )
      # where("name like ?", ["%football%", "%wrestling%"])
